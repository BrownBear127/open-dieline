...
> 單片視圖句殼·法蘭三案對照選 A·2026-07-16）——補 M1 佈線期 `${label.en} view` 硬編碼
...
> EN 預設語言（D2）；zh＝現值逐字；**Tier B EN＝M0.6 SOL 起草→小稜複核→回寫附件
> →法蘭抽查**（SOL readiness：文案裁決不留實作 task）

## 規則（先簽這節）

1. **Tier A**＝chrome＋短 label：EN 終稿在本表（法蘭簽核即凍結）。
...
5. **版本字樣（已簽核·2026-07-16）**：版本號動態注入 `import pkg from
   '../../package.json'`（自 `src/ui/` 起算的正確相對路徑——SOL I4 修正原稿 `../` 會
...
| modal.version | v{version} | v{version} | 規則 5：動態注入·「開發測試版」刪（⚠ 法蘭簽） |
...
- chrome/UI key → `src/i18n/` 字典（`{ en, zh }`·單一檔）；**組合句用 message-template**（整句 key＋具名參數·每語言自持模板——EN 語序重排需要·SOL M0.6 實證·禁字串拼接）
...
6. Tier B 的分層辦法（EN 由 M1 SOL 起草·小稜複核·回寫）
