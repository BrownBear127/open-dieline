...
| key | zh（逐字） | audit # | 備註 |
|---|---|---|---|
| modal.body.p1 | 一個刀模（dieline）快速原型工具——把包裝盒的結構知識做成可以調參數、可以列印試摺的工具，產出接近真實刀模的原型展示。<br>幾何規則以真實生產刀模逆向量測校準；正式生產前請與刀模廠校對打樣。 | #4 | 原文含換行＋全形破折號，逐字抄 |
...
| fold.loadFailed | 3D 摺盒預覽載入失敗。 | — | P3 M2 新增·2026-07-17 簽核 |
...
| 6 | impositionIcons.tsx 內嵌數字「31」「25」「27」 | — | #36-38 | review-confirm（無需新 key） | 紙規圖示 SVG 內嵌的裝飾性簡碼數字，屬純數值（比照 §E「純數值模板不進字典」邏輯），與 `imp.sheet.preset.{i}` 的完整規格文字（`31"×43"` 等，#125-127）是不同的兩組字串；判定不需獨立字典 key，僅供 實作輪 確認此判斷 |
...
| 23 | `Download report`（footer btn，line 517） | 無對應=mock 示意 | audit 全文未見任何「下載報告」功能或按鈕文字（ImpositionResults.tsx 只有拼版卡片＋SVG/DXF 匯出），疑似 mock 超前示意的未實作功能，需 實作輪 確認是否為真實需求或純示意 |
...
**次要/待確認項（非文字內容錯誤，供 實作輪 參考，不列入上表 27 條主計數）：**

- `full set`（plate-label 內嵌小寫，line 365）vs `canvas.view.fullSet` 終稿 Title Case `Full set`——大小寫風格差異，可能是模板拼接時的刻意小寫（label 聲部語境），建議 實作輪 確認是否需統一。
- `aria-label="Zoom out"` / `"Zoom in"`（zoom 按鈕，lines 372-373）——audit 未記錄畫布縮放鈕的 aria-label，現行 inventory 也未建對應 key；可能是 mock 新增的 a11y 屬性，需 實作輪 確認是否要補 key，或維持沿用視覺字元本身（`canvas.zoom.in/out`）作為 aria 內容。
...
無遺漏、無雙重計數衝突。唯一需要裁決的懸而未決項＝節2 的 6 條補漏＋節3
