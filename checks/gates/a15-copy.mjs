...
const VERIFIED_TIER_B_SECTIONS = ['B3', 'B4', 'B6', 'B8', 'B9', 'B10'];

// B10 是末號 B 節——無 B11 可作節尾。顯式指定兩檔各自的下一個標題：
...
function rowsForSection(markdown, section, endOverride) {
  const next = endOverride ?? `B${Number(section.slice(1)) + 1}`;
...
function addTierB(keys, values, expansion, tierBEnglish) {
  for (const section of VERIFIED_TIER_B_SECTIONS) {
    const ends = SECTION_END_OVERRIDES[section];
    const expansionRows = rowsForSection(expansion, section, ends?.expansion);
    const englishRows = rowsForSection(tierBEnglish, section, ends?.tierb);
