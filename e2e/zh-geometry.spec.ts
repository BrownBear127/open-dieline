...
// F4（H4，法蘭裁決 A 的收斂實作，2026-07-16）: the imposition disclaimer line
...
// F4（H4，法蘭裁決 A 的收斂實作）: modal body copy length differs per language (title `h2`
...
test('zh strict geometry parity: modal container', async ({ page }) => {
  ...
  // F4（H4，法蘭裁決 A 的收斂實作）: the card's *natural* (pre-clamp) height delta —
