 ...
 *
 * **重定義背景**：法蘭實際操作樣張後裁決，原本的「L 形斷面落地圍框」（帶狀攤平＋翻邊＋
 * 黏合 tab，錨定上蓋內淨）是實際不會用的版本。正確形式（法蘭定案）：
...
/**
 * 內襯導出鏈（2026-07-09 T7 gate 重定義；brief 逐字公式）——底面錨定＝下盒內淨，
 * 無獨立尺寸參數：
...
export interface LinerOpts extends LinerFrameInputs {
  /** 腳架深度（架高量）——四邊共用同一值（法蘭定案：單一參數，不分長短壁）。 */
  flapDepth: number;
  idPrefix: string;
  offsetX: number;
  offsetY: number;
...
/** 底面＋翼深兩條尺寸標註（brief：標「底面 padL×padW＋翼深」）；offset 皆外推，見 DIM_OFFSET 註解。 */
