 ...
 * along=topEndAlong 處以 T 型交會銜接、不重疊（見 開發紀錄 對算過程；F4/F6-A
 ...
 * 實證——Fix 4·2026-07-11 review M4：先前殘留 height−thickness 舊公式是 T1 參數遷移
 ...
 * 真交叉（cut 自撞掃描 24 組合曾在此抓到，見 開發紀錄）。
 */
...
function snapALongAnchor(raw: number, anchors: LongWallAnchors): number {
  ...
  // 獨立公式測試（開發紀錄：0.0007mm 量測殘差曾在此讓 bbox 錨對不上）。
 ...
 *
 * **F1 review Fix（2026-07-11，review 指出＋獨立重新量測 P 四角確認)**：原實作誤用通用
