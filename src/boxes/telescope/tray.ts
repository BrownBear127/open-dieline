...
/** 薄壁角撐讓位弧半徑，spec 明列固定值（非比例）。 */
const GUSSET_B_RELIEF_RADIUS = 5.0;

/** 薄壁角撐頂小弧半徑，spec 明列固定值（bezier 反解近似值，非比例）。 */
 ...
 * 復刻自附錄 A 片座標（角落/中點/tip/V3/V4 五點結構逐一驗證，見 開發紀錄）。
 */
 ...
 * 90° 案例驗算核對，見 開發紀錄）。sweep 由 diag×dir 外積正負決定圓角凹凸
 * 方向，對四個角落（sx/sy 四種組合）都自動算出正確方向，不需個別鏡射特判。
 * 圓角不強求與 45° crease 完全相切（tip 端可能有極小轉角）——這是「先理解復刻、
 * 再參數化」的簡化（spec 允許，T5 對帳驗證最終正確性），已用四種 H/t/platform
