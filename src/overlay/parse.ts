...
/**
 * 矩陣線性部分是否「等比縮放＋旋轉（含鏡射）」：a²+b²＝c²+d² 且 a·c+b·d＝0（spec 判準）。
 *
 * 容差刻意用 `EPS × max(norm1,norm2,1)`（等比於矩陣係數量級）而非 spec 字面的固定 `> EPS`：
 * 固定 EPS 在座標/scale 量級大時（如 scale(1e6)）會把本應視為等比的矩陣誤判為非等比而跑去
 * 展開成 bezier（純屬多餘），量級小時又可能把非等比誤判為等比。兩種寫法只在矩陣係數量級落在
 * ~1e6 以上（spec 未定義 EPS 值本身、也不是這類刀模檔案會出現的物理量級）才會分歧，故不影響
 ...
 *  c1=p0+2/3(q−p0)、c2=p3+2/3(q−p3)（spec 公式，等價於標準 quadratic-to-cubic degree elevation）。 */
