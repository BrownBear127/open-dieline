 ...
 * bezier 取端點距，arc 也取起訖點的弦長（非弧長）。bezier 的控制點不影響這個值（spec 明文
 * 「都是弦、不是弧長」，只看兩端點）。
 *
...
// 同一個 5° 步進值。geometry.ts 不在本輪 可修改檔案清單內、該函式也未 export，這裡照抄
// 一份精簡版（只回傳點陣列，不像 geometry.ts 那份還要組成 LineSegment[]）。取捨：兩份程式碼
// 重複的維護成本，換取不擴大本輪 的檔案改動範圍；若未來 geometry.ts 的取樣邏輯調整，這裡
// 不會自動同步，需要人工對照更新。
 ...
 *  既有 `flattenBezier`（`core/geometry.ts` 已 export，T5 spec 明文可直接用）。 */
 ...
 * 跳過、永遠不會被判定為「最近」——呼應 spec「選中零長段→忽略點擊」：與其讓使用者先選中
