 ...
 * 畫面反應。task-5-brief.md 的「Modify」檔案清單原本沒列 App.tsx（T4 的 brief 有明確列——
 * 見 task-4-report.md），但這個功能結構上必須有 Canvas→App 的回寫管道；這裡只加這一個
 * prop（重用既有的 `setOverlayState`，不新增任何 state 或邏輯），是能讓 T5 實際運作的
 * 最小必要改動，已在 task-5-report.md 標註為對 brief 檔案清單的刻意偏離，供覆核。
 */
