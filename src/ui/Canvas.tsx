...
/**
 * 多片盒型（天地盒等）預設縮放倍率（T7 樣張 gate 第一輪驗收反饋修 3·2026-07-09）：
...
export function Canvas({
  ...
  // 隨每次 mousemove 更新，才能算出連續多次 mousemove 的逐次 delta（brief：拖曳過程 offset
  ...
  // 按 Esc 該退出校準模式、選取狀態原封不動（brief「校準模式優先」的同一順位道理，上一個
  ...
  // T3 拖曳分流條件（brief：選中 overlay 層＝selectedOverlayId 非 null 且該層 visible；
  // 校準模式優先於拖曳分流，校準中不拖 overlay）。刻意不額外檢查
  // `selectedLayer.segments.length`——拖曳只是更新 offsetX/offsetY 兩個數字，不像
  // hit-test 需要真的走訪 segments，brief 規格也只講 selectedOverlayId／visible 兩個條件，
  // 不無中生有加這層 gate。
  ...
  const handleMouseDown = (e: ReactMouseEvent<HTMLDivElement>) => {
    ...
    // 分流（brief「校準>選中拖曳>pan」）：canDragOverlay 已經把「校準中」排除在外（定義見
   ...
   *
   * mm delta 換算（brief 明文的易錯點，容易寫錯的地方）：只除以畫布 `scale`（zoom），
  ...
  return (
    <section className="bench flex-1 h-full">
      ...
      <div
        ref={containerRef}
        // 游標分流（T3 新增 cursor-move 分支）：校準模式十字準星（brief UI 規格；pan/zoom
