 ...
 * `result.pieces` 存在但 `state.pieceId` 暫時找不到對應片（T4 的 effect 尚未來得及修正的
 * 過渡 render）時，`piece` 為 `undefined`，`manufacturingBounds(result, undefined)` 會退回
 * 「全版」bounds——安全但語意上非最終狀態，只是過渡一幀，不會 crash。
 *
...
const USABLE_ZONE_FILL = '#ffffff'; // 可用區（扣咬口後，instances 實際落點）
...
export function ImpositionView({ result, state, onChange }: ImpositionViewProps) {
  ...
  const workingSheet = imposition.ok ? imposition.sheet : null;
  const previewPathList = previewPaths(result, piece ?? null);

  const deg0Instances = imposition.ok
    ? instanceTransforms(0, imposition.deg0.cols, imposition.deg0.rows, mb, state.gripper, state.gap)
    : [];
  const deg90Instances = imposition.ok
      ...
      <div className="flex-1 flex flex-col gap-3">
        ...
        <div className="flex gap-3">
          <DirectionCard
            dirDeg={0}
            label="0°"
            direction={imposition.ok ? imposition.deg0 : null}
          ...
          <DirectionCard
            dirDeg={90}
            label="90°"
            direction={imposition.ok ? imposition.deg90 : null}
