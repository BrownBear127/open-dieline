 ...
 *
 * 單位下拉變更重算 scale：brief 原文「僅在未做過點選校準時」——T4 尚無校準機制（T5 才加
 * `calibrateScale` 與 Canvas 校準 hit-test），這條優先順序在本 task 恆真，因此實作上＝
...
export function OverlayPanel({ overlayState, onOverlayStateChange, targetBounds }: OverlayPanelProps) {
  const [unit, setUnit] = useState<OverlayUnit>('pt'); // 法蘭生產檔慣例，見 brief UI 規格
