...
// 不篩子集（2026-07-11 review M1 修正——舊版只收集上述 7 個新降級 id、把既有 module
...
// 的關係：本檔是 實作輪 指定的正式落地位置（spec 原文明點 param-sweep 機制）。下面
...
// 表一致，且與 Task 4 的既有測試一致——沒有發現不一致，因此沒有需要回報 實作輪 的落差
...
describe('telescope（天地盒）：縮放降級矩陣 S1-S7（spec §驗收 4，Slice 5 Task 5）', () => {
   ...
   * （16 條）中實際觸發（check().ok===false）的 id 完整集合——不篩子集（2026-07-11
  ...
  // - tongue-flap-fits（既有 module invariant，2026-07-11 review M1 修正新增）：base
  ...
  it('全組（S1-S7）：generate 不 throw、無 NaN、bounds 有限、全部既有 module invariant.check() 不 throw', () => {
    ...
    for (const [label, overrides] of cases) {
      ...
      // fired-set 比對釘住（2026-07-11 review M1 修正：assertDegradationMatrix 不再
