...
describe('reverseTuckEnd', () => {
  ...
  it('等價驗證：與前身輸出在 normalized Segment 層一致，cut/crease/dimension 分線型比對（spec §4.1，Fix Round 1 #1；FX6 補強 dimension 座標）', () => {
    ...
    // （已用一次性變造腳本驗證此嚴格性差異，證據見 task-6-report.md「Fix Round 1」。）
    // thickness: 0 是 t=0 錨定的核心——fixture 由不吃補償的前身函式產生，只有 t=0 時
    // 我們的補償公式全部歸零、幾何才會與 fixture 逐位元等價（brief Step 1）。
  ...
  // 已用 L=40,W=90 的區分性參數實測驗證（見 task-6-report.md Fix Round 1）：tuckLock
