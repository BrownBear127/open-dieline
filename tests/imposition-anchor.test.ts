...
describe('computeImposition — RTE 三 preset 錨（profile-spacing spec 驗收 1：27×39/31×43 既有單元/UI 級錨的整合鏈引用確認＋25×35 新錨）', () => {
  ...
  it('31×43（787×1092，App 預設紙規）：0°=3×5=15、90°=3×4=12——tests/imposition-app.test.tsx T4 既有 UI 級錨（探針腳本 獨立重導 bit-exact）的整合鏈引用確認', () => {
...
//
// 斷言門檻＝`sampleDistance ≥ gap`（不是 `gap − errorBound`——2026-07 review H1
 ...
 *  漏掉真正最緊的那一對，見 2026-07 review H2）。這兩個案例沒有餘裕可言，正確性不能靠
 ...
 *  minDistance 的排序+二分視窗剪枝，單次呼叫約 70-90ms（開發紀錄 記錄了
 ...
 *  誤差＝半弦長＋弦高（sagitta，spec 明文公式 `r(1−cos(θ/2))`，兩者同方向疊加，保守）。 */
 ...
 *
 *  改用「控制多邊形到『有限弦段』的最大距離」（2026-07 窄 re-review M 第二輪 finding
 ...
 *
 *  **第二輪修正**（round-1 版本量到「無限延伸直線」而非有限弦段，被 2026-07 review 窄
 ...
 *  「round-2 反例」測試的密集抽樣量測，以及本檔 M 驗紅記錄）。根源：「到無限直線的
...
/** 兩點集最小歐氏距離——排序＋二分搜尋視窗剪枝（spec「或帶正確剪枝——|Δx|≥門檻可跳」的
...
describe('computeImposition — 間距不變式（profile-spacing spec 驗收 2：幾何級證明，獨立於 core/profile.ts 內部近似手法）', () => {
  it('RTE 27×39 行縮：第 n 行與第 n+1 行（垂直位移 strideY）任兩邊界點對歐氏距離 ≥ gap − 離散誤差上界（cols 軸未收縮，異欄件由矩形 strideX 分離已保證 ≥gap——見 spec F3「禁止雙向同時收縮」的對角安全論證，只需驗同欄相鄰行）', () => {
    ...
    // 實測（開發紀錄 記錄完整推導）：maxChordMM=0.1mm → maxPointErrorMM≈0.0501mm、
    ...
    // 斷言門檻＝gap（不是 gap-errorBound，見檔頭「誤差方向推論」2026-07 review H1 訂正）。
    expect(observed).toBeGreaterThanOrEqual(gap);
    expect(observed).toBeCloseTo(3.8, 1); // 迴歸錨：真實最小距離的量級鎖定，抓未來意外變動（非本不變式斷言的必要部分）
  });

  it('Z-notch 正補排案例（POSITIVE_FILL_INPUT）DEG0：主格點邊界行（最後一行，全部欄）× 底條帶全部 instance 窮舉——收縮主格點×補排條帶的相鄰件對驗證幾何級間距（spec 驗收 2「擴展主格點×條帶件對」；2026-07 review H2：原本只驗第 0 欄×第 0 件（10.44mm），獨立枚舉全部件對後整體最小值恰＝gap，見下方斷言與 report 附錄的完整枚舉記錄）', () => {
    ...
    // 斷言門檻＝gap（見檔頭「誤差方向推論」2026-07 review H1 訂正，同一修正也適用這裡；
    ...
    // （開發紀錄 附錄記錄完整枚舉數字＋mutation 驗紅）；這條邊界貼齊 gap 沒有多餘
    // 餘裕，digits=1 只鎖「差不多剛好=gap」這個事實，避免對浮點尾數過度脆弱。
    expect(globalMin).toBeCloseTo(3, 1);
  });

  it('Z-notch 正補排案例 DEG90：主格點邊界欄（最後一欄，全部行）× 右條帶全部 instance 窮舉——同一組輸入的另一方向，證明檢查邏輯不是只在 DEG0 湊巧成立（2026-07 review H2：與 DEG0 同一缺陷，原本只驗第 0 行×第 0 件，獨立枚舉後全域最小值同樣恰＝gap，命中在不同的一組件對）', () => {
    const shrunk = computeProfileStrides(Z_NOTCH_SEGMENTS, Z_NOTCH_GAP);
    const result = computeImposition({ ...POSITIVE_FILL_INPUT, shrunk });
    assertOk(result);
    ...
    // 斷言門檻＝gap（同 DEG0／檔頭「誤差方向推論」2026-07 review H1 訂正）。
    expect(globalMin, `global min at ${globalMinAt}`).toBeGreaterThanOrEqual(Z_NOTCH_GAP);
    // 這組對恰好卡在 gap 邊界本身（開發紀錄 記錄推導：strip 起點公式 usedW+gap 與
...
describe('discretizeBezier 的獨立控制多邊形終止判準（2026-07 review M finding：core/geometry.ts flattenBezier 的中點抽樣判準可被對稱 S 形曲線騙過，本檔已改用不受此類曲線影響的判準，見上方 discretizeBezier 群組函式 docblock）', () => {
  ...
  it('round-2 反例：控制點與弦共線但投影落在有限弦段外——到「無限直線」的距離讀出 0，到「有限弦段」的距離才讀出真實偏離（2026-07 窄 re-review M 第二輪 finding：round-1 版 controlPolygonHeight 用 distToChordLine 量控制點到「弦所在無限延伸直線」的垂直距離，這個反例的控制點恰好也在那條直線上，垂直距離恆為 0，即使控制點的座標遠遠超出弦段本身的範圍，判準也讀不出來，見下方函式 docblock「第二輪修正」段的完整推導）', () => {
    ...
    // 曲線的客觀事實，跟判準怎麼實作無關（斷言 1：量級證據，spec 記錄真實偏離 ≈28.843mm，
...
describe('computeImposition — telescope production-P 逐片迴歸鎖（profile-spacing spec 驗收 3：與 0f837fa〔profile-spacing slice 開工前基線〕逐字相同）', () => {
  // 0f837fa 基線數字的取得方式（開發紀錄 記錄完整過程）：用 git worktree checkout
  ...
  it('下盒（base）@ 27×39：與 0f837fa 基線逐字相同（cols/rows/gridCount/fillSplit/bottomFill/rightFill/count/totalCount/utilization 全欄——0f837fa 的 DirectionResult 沒有 strideX/Y／usedW/H／spacingAxis，故不比對那四欄）；帶 shrunk 與不帶結果 toEqual（十字形件零收益，補齊 base 片在額外 preset 上的樣本——T2「telescope 退化」只驗過 1200×1200 人造大紙規）', () => {
    ...
    expect(withoutShrunk.deg0.utilization).toBe(0.5399715809633642); // 逐位元精確值，非只是數值接近（2026-07 review L2）
    ...
    expect(withoutShrunk.deg90.utilization).toBe(0.5399715809633642); // 逐位元精確值，非只是數值接近（2026-07 review L2）
  });
  ...
  it('上蓋（lid）@ 25×35：與 0f837fa 基線逐字相同；帶 shrunk 與不帶結果 toEqual（補齊 lid 片樣本——這個 preset 剛好讓 deg0.bottomFill 算出非零 count=1，一併驗證「有 L 形補排」路徑在迴歸鎖下也不漂移，不是只覆蓋全零退化的情形）', () => {
    ...
    expect(withoutShrunk.deg0.utilization).toBe(0.5416641187568089); // 逐位元精確值，非只是數值接近（2026-07 review L2）
    expect(withoutShrunk.deg90).toMatchObject({
      cols: 1,
      rows: 2,
      gridCount: 2,
    ...
    expect(withoutShrunk.deg90.utilization).toBe(0.5416641187568089); // 逐位元精確值，非只是數值接近（2026-07 review L2）
  });
});
