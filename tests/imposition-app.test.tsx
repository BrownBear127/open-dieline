...
const sharedLidBox: BoxModule = {
  meta: {
    id: 'test-shared-lid-box',
    name: { zh: '測試共享lid盒', en: 'Shared-lid test box' },
    intro: { zh: '', en: '' },
    topology: 'nested',
  },
  params: [
    {
      ...
      description: {
        zh: '測試參數：本盒型幾何寫死常數，generate() 不消費它',
        en: 'Synthetic parameter unused by the fixed test geometry.',
      },
    },
  ],
  invariants: [],
  generate: () => ({
    ...
    pieces: [
      { id: 'new-first', label: { zh: '新片一', en: 'New first piece' }, pathIds: ['nf-p0'], textIds: [], bounds: { minX: 0, maxX: 10, minY: 0, maxY: 0 } },
      { id: 'lid', label: { zh: '共享lid片', en: 'Shared lid piece' }, pathIds: ['lid-p0'], textIds: [], bounds: { minX: 0, maxX: 10, minY: 20, maxY: 20 } },
    ],
  }),
};
...
describe('App：模式切換顯隱（spec「組裝」段：LayersPanel/ExportBar ↔ 拼版控制、Canvas ↔ 拼版預覽、ParamPanel／盒型選擇不隨模式隱藏）', () => {
  ...
  it('預設（刀模設計）模式：LayersPanel／ExportBar／ParamPanel 可見，拼版控制／結果不可見', () => {
    ...
    expect(screen.queryByRole('group', { name: t('imp.title') })).not.toBeInTheDocument();
  ...
  it('切到拼版估算：LayersPanel／ExportBar 消失、ImpositionControls／ImpositionResults 出現，ParamPanel／盒型選擇仍在', () => {
    ...
    expect(workspace).toHaveClass('min-h-0', 'flex-1', 'flex');
    expect(workspace).not.toHaveClass('overflow-y-auto', 'p-6', 'bg-white');
    const bench = workspace?.querySelector(':scope > section.bench');
    expect(bench).not.toBeNull();
    expect(bench?.querySelector(':scope > .imp-toolbar')).toBe(screen.getByRole('group', { name: t('imp.title') }));
...
describe('App：拼版預設值與即時性（spec 驗收條件 1 數值錨＋驗收條件 9）', () => {
  it('RTE 預設參數進拼版模式：0°＝3列×5行＝15模（行縮，spacingAxis=rows）、90°＝3列×4行＝12模（列縮，spacingAxis=cols）——profile-spacing slice 生效後 31×43 預設紙規新錨，取代 gate round 1 T1 舊矩形錨 12/12（plan v1.2 N1：App 預設紙規是 31×43 非 27×39，舊值 12/12 屬 31×43 矩形排列；實作輪 手算參考 0° 3×5=15＋補排 0／90° 3×4=12＋補排 0，實作輪 以 探針腳本 獨立重導 bit-exact 吻合，見 開發紀錄）', () => {
    ...
    expect(within(card0).getByText(t('imp.grid.formula', { cols: 3, rows: 5, fillSuffix: '', count: 15 }))).toBeInTheDocument();
    expect(within(card90).getByText(t('imp.grid.formula', { cols: 3, rows: 4, fillSuffix: '', count: 12 }))).toBeInTheDocument();
  });

  it('90° 卡在 31×43 預設紙規＋profile-spacing 生效後為純列縮主格點（3×4=12，無 L 形補排）：gate round 1 T1 舊錨（gridCount8＋補4=12）在本紙規已被列縮擇優取代——列縮 stride 已讓 3×4=12 直接鋪滿，pickFillSplit 兩條帶 count 皆算出 0（見 探針腳本）。isTruncated 假陽性回歸（count>gridCount 但兩者皆≤renderedCount 不應誤判截斷）的底層程式碼路徑未變動，此規律的矩形件版本仍由 tests/imposition-view.test.tsx「production-chain usedW/usedH 一致性」等測試（不受本 slice 影響）持續覆蓋；這裡改鎖新常態——12 個 instance 全數落在主格點、全數 rotate(90)（純 90° 方向、無補排件轉回 0° 的互補分支）', () => {
    render(<App />);
    switchToImposition();

    expect(screen.queryByText(t('imp.previewSimplified'))).toBeNull();
  ...
  it('拼版模式下直接調整盒參數（L，不需切回設計模式——ParamPanel 兩模式皆可調）→ 方向卡即時重算（驗收條件9 抽驗一組）', () => {
    ...
    const lengthInput = screen.getByLabelText(paramLabel(reverseTuckEnd, 'L'));
...
describe('App：模式往返 state 保留（spec F6「模式往返」：逐欄列舉，不用「任何參數」概括；review Medium 2——完整旅程逐欄鎖住，另補 state 分離與 overlay 保留兩項斷言）', () => {
  it('逐欄寫入非預設值（盒型參數／拼版件選擇／紙規 preset／自訂 W-H／方向／裁切 cutV-cutH／旋轉 allowRotate／咬口／gap）→ design↔imposition 往返 → 逐欄斷言保留；並驗證 impositionState.pieceId 與 selectedPieceId 互不污染、overlay 資料往返後仍存在（T4：紙規/方向/裁切改按鈕點擊，取代舊 select 互動；review 覆蓋缺口——allowRotate 補為第 10 欄）', async () => {
    ...
    const boxSelect = screen.getByLabelText(t('console.boxStyle'));
    fireEvent.change(boxSelect, { target: { value: 'telescope' } });
    const lidButton = await screen.findByRole('button', { name: 'Lid' }); // 等切盒完成（telescope 的片切換鈕出現）
    ...
    const baseLengthInput = screen.getByLabelText(paramLabel(telescope, 'baseLength')) as HTMLInputElement;
    fireEvent.change(baseLengthInput, { target: { value: '250' } });
    expect(baseLengthInput.value).toBe('250');

    switchToImposition();

    const pieceSelect = screen.getByLabelText(t('imp.piece')) as HTMLSelectElement;
    ...
    const paperGroup = within(screen.getByText(t('imp.sheet')).closest('.imp-group') as HTMLElement);
    ...
    expect(paperGroup.getByRole('button', { name: t('imp.sheet.custom') })).toHaveAttribute('aria-pressed', 'true');

    const customWInput = screen.getByLabelText(t('imp.sheet.w')) as HTMLInputElement;
    const customHInput = screen.getByLabelText(t('imp.sheet.h')) as HTMLInputElement;
    ...
    fireEvent.click(
      within(screen.getByText(t('imp.orient')).closest('.imp-group') as HTMLElement).getByRole('button', {
        name: t('imp.orient.landscape'),
      }),
    );
    ...
    fireEvent.click(
      within(screen.getByText(t('imp.halving')).closest('.imp-group') as HTMLElement).getByRole('button', {
        name: t('imp.halving.v'),
      }),
    );

    // ── 旋轉：allowRotate(預設 true)→false（第 10 欄，T4：按鈕點擊，取代 T1 checkbox；
    //    review 覆蓋缺口——F6 逐欄旅程先前漏測這欄） ──
    fireEvent.click(screen.getByRole('button', { name: t('imp.rotate.allow') }));

    // ── 咬口：20(預設)→15 ──
    const gripperInput = screen.getByLabelText(t('imp.gripper')) as HTMLInputElement;
    fireEvent.change(gripperInput, { target: { value: '15' } });

    // ── 刀線間距(gap)：3=MIN_GAP_MM(預設)→6 ──
    const gapInput = screen.getByLabelText(t('imp.gutter')) as HTMLInputElement;
    ...
    expect(screen.getByRole('button', { name: 'Lid' })).toHaveAttribute('aria-pressed', 'true');
    expect((screen.getByLabelText(paramLabel(telescope, 'baseLength')) as HTMLInputElement).value).toBe('250'); // 盒型參數（values）保留
    ...
    expect((screen.getByLabelText(t('imp.piece')) as HTMLSelectElement).value).toBe('liner');
    expect(
      within(screen.getByText(t('imp.sheet')).closest('.imp-group') as HTMLElement).getByRole('button', { name: t('imp.sheet.custom') }),
    ).toHaveAttribute(
      'aria-pressed',
      'true',
    );
    expect((screen.getByLabelText(t('imp.sheet.w')) as HTMLInputElement).value).toBe('900');
    expect((screen.getByLabelText(t('imp.sheet.h')) as HTMLInputElement).value).toBe('1200');
    const finalOrientationGroup = within(screen.getByText(t('imp.orient')).closest('.imp-group') as HTMLElement);
    expect(finalOrientationGroup.getByRole('button', { name: t('imp.orient.landscape') })).toHaveAttribute('aria-pressed', 'true');
    expect(finalOrientationGroup.getByRole('button', { name: t('imp.orient.portrait') })).toHaveAttribute('aria-pressed', 'false');
    const finalCutGroup = within(screen.getByText(t('imp.halving')).closest('.imp-group') as HTMLElement);
    expect(finalCutGroup.getByRole('button', { name: t('imp.halving.v') })).toHaveAttribute('aria-pressed', 'true');
    expect(finalCutGroup.getByRole('button', { name: t('imp.halving.h') })).toHaveAttribute('aria-pressed', 'false');
    expect(screen.getByRole('button', { name: t('imp.rotate.allow') })).toHaveAttribute('aria-pressed', 'false');
    expect((screen.getByLabelText(t('imp.gripper')) as HTMLInputElement).value).toBe('15');
    expect((screen.getByLabelText(t('imp.gutter')) as HTMLInputElement).value).toBe('6');
  });
});
...
describe('App：impositionState.pieceId 生命週期（spec F6「state 分離」／「預設值」／「失效 fallback」／「RTE」）', () => {
  it('切到 telescope 並進拼版模式：件下拉初值為 pieces[0]（下盒 base），非 null／非其他片', async () => {
    ...
    await waitFor(() => {
      expect((screen.getByLabelText(t('imp.piece')) as HTMLSelectElement).value).toBe('base');
    });
  });
  ...
  it('天地盒選內襯(liner)後關閉「內襯墊片」(linerEnabled=false)：件下拉即時 fallback 回下盒(base)——不需切模式，ParamPanel 在拼版模式仍可調', async () => {
    ...
    fireEvent.click(screen.getByLabelText(paramLabel(telescope, 'linerEnabled'))); // 關閉 linerEnabled → 'liner' 片消失
  ...
  it('RTE（result.pieces undefined）恆顯示「整件」、無下拉；換盒型 fallback：telescope 選內襯→切到 RTE→切回 telescope，不殘留舊 liner 選擇、fallback 回下盒(base)', async () => {
    ...
    expect(screen.getByText(t('imp.piece.whole'))).toBeInTheDocument();

    fireEvent.change(boxSelect, { target: { value: 'telescope' } });
    await waitFor(() => {
      expect((screen.getByLabelText(t('imp.piece')) as HTMLSelectElement).value).toBe('base'); // 不殘留 'liner'
    });
  });
  ...
  it('換盒型即歸位 pieces[0]，即使新盒型的非首片沿用舊 id（review Medium 1 回歸 fixture）：telescope 選「上蓋」(lid) 後切到 test-shared-lid-box（pieces=[new-first, lid]，"lid" 落在第二位），不得停留在同名的 lid，須歸位新盒的 pieces[0]="new-first"', async () => {
    ...
    await waitFor(() => {
      expect((screen.getByLabelText(t('imp.piece')) as HTMLSelectElement).value).toBe('new-first');
    });
  });
});
...
describe('App：拼版模式與 calibrating 互斥（spec F6「校準互斥」）', () => {
  it('calibrating=true 時進拼版模式：Canvas／LayersPanel 卸載、calibrating 被強制關閉；退出拼版後返回設計模式不復活（鈕字回「校準」而非「取消校準」，提示條不自動重現）', async () => {
    ...
    switchToImposition();
    expect(screen.getByRole('group', { name: t('imp.title') })).toBeInTheDocument(); // 確認真的進了拼版模式（LayersPanel 已卸載）

    switchToDesign();
    expect(screen.queryByText(t('canvas.calibrate.hint'))).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: t('overlay.calibrate') })).toBeInTheDocument(); // 非「取消校準」→ calibrating 沒有復活
  });
});
...
describe('App：LayersState 跨模式／跨盒型組合旅程（final 迴歸 review Low 3——設計圖層狀態、overlay、雙 piece state 在完整往返後全保留的永久保護）', () => {
  it('匯 overlay → 關切割線與尺寸標註 → 進拼版（製造輪廓照畫、overlay 不出現）→ 切盒型＋改拼版選片 → 回設計：兩圖層仍關、overlay 仍在、cut 仍不畫', async () => {
    ...
    switchToDesign();
    expect((screen.getByLabelText(t('layers.cut')) as HTMLInputElement).checked).toBe(false);
