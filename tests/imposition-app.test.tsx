...
describe('App：拼版預設值與即時性（spec 驗收條件 1 數值錨＋驗收條件 9）', () => {
  ...
  it('Fix 1（isTruncated 假陽性回歸）：RTE 預設參數＋App 預設 allowRotate:true 下，90° 卡 count=12 建立在主格點 gridCount=8 之上（補排 1×4=4 件）——預覽只畫 8 個主格點 instances，不應誤判「數量過大」（改前：count(12) > instances.length(8) 恆真，即使遠低於 MAX_PREVIEW_INSTANCES=500 也誤顯示簡化提示）', () => {
    render(<App />);
    switchToImposition();
...
describe('App：模式往返 state 保留（spec F6「模式往返」：逐欄列舉，不用「任何參數」概括；review Medium 2——完整旅程逐欄鎖住，另補 state 分離與 overlay 保留兩項斷言）', () => {
  it('逐欄寫入非預設值（盒型參數／拼版件選擇／紙規 preset／自訂 W-H／方向／裁切 cutV-cutH／咬口／gap）→ design↔imposition 往返 → 逐欄斷言保留；並驗證 impositionState.pieceId 與 selectedPieceId 互不污染、overlay 資料往返後仍存在（T4：紙規/方向/裁切改按鈕點擊，取代舊 select 互動）', async () => {
    render(<App />);
