...
describe('App：拼版預設值與即時性（spec 驗收條件 1 數值錨＋驗收條件 9）', () => {
  ...
  it('Fix 1（isTruncated 假陽性回歸）：RTE 預設參數＋App 預設 allowRotate:true 下，90° 卡 count=12 建立在主格點 gridCount=8 之上（補排 1×4=4 件）——預覽實際畫出全部 12 個 instances（不再是舊註解寫的「只畫 8 個主格點」，補排渲染早已補上），不應誤判「數量過大」（改前：count(12) > instances.length(8) 恆真，即使遠低於 MAX_PREVIEW_INSTANCES=500 也誤顯示簡化提示）；同時鎖住 90° 卡的 instance 組成防止再度退化——12 個 instance 中 8 個 transform 含 rotate(90)（主格點，本身已是 90° 方向）、4 個不含（補排件轉回 0°、與主格點互補，見 core pickFillSplit docblock；數字用獨立渲染探針驗證，非憑空推算）。0° 卡同一時刻是 12 個主格點、0 補排、全不轉（無 L 形剩餘空間可補），這裡不重複斷言，只鎖 90° 卡', () => {
    render(<App />);
    switchToImposition();
