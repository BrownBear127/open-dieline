...
function applyTransform(transform: string, p: Pt): Pt {
  ...
  return tokens.reduceRight((pt, t) => {
    ...
    // rotate(90)：本模組只產生 90°，SVG 順時針 (x,y)→(−y,x)（spec 明定的旋轉方向）。
...
describe('directionInstances — 主格點＋補排條帶排列（bottom-full／right-full 條帶原點公式）', () => {
  ...
  // ——spec RED 條款明定的公式，逐字硬編碼在下面斷言，不由被測函式導出（spec F8）。
...
describe('directionInstances — budget 邊界（review High 2：正規化並硬限 0…MAX_PREVIEW_INSTANCES）', () => {
  const mb: Bounds = { minX: 1, maxX: 11, minY: 2, maxY: 8 };
  const gripper = 5;
  const gap = 2;
  const direction: DirectionResult = {
...
describe('directionInstances — 正數補排＋收縮並存（主格點 stride／補排矩形／條帶起點 usedW-usedH 三件事同時成立）', () => {
  ...
  it('右條帶補排件（deg90 對稱案例，rightFill.count=4）：起點讀 direction.usedW(343)+gap，不是本地重算（2×200+1×3+gap＝406≠正確值 346）；位移用旋轉後矩形 fillCellW+gap——deg0 案例 rightFill 恆為 0，本測試補上 usedW／右條帶呼叫位的鑑別力', () => {
    const direction90 = result.deg90;
    const instances = directionInstances(90, direction90, mb, 0, Z_NOTCH_GAP, 500);
    expect(instances).toHaveLength(20); // 16 主格點 + 0 底條帶 + 4 右條帶——與 core count 一致
    const right0 = instances[16]!;
    const right1 = instances[17]!;
    expect(right0.cellX).toBe(346); // gripper(0) + usedW(343) + gap(3)
    expect(right0.cellX).not.toBe(406); // 本地重算的錯誤值（同 deg0 案例同構：2×200+1×3+3）
    expect(right1.cellX - right0.cellX).toBe(53); // fillCellW(=mb w=50，fillDir=0 下)+gap(3)——非主格點 stride(143/53)
  });
});
...
describe('sectionOffsets — 子紙左上角偏移（全紙座標系），四情境＋順序固定左上→右上→左下→右下', () => {
  ...
  it('讀旗標不猜尺寸：w===fullW（尺寸完全看不出裁切痕跡）時仍依 cutV=true 回傳 2 段', () => {
    ...
    // 「讀旗標」不是「讀尺寸差」——用尺寸差反推會在這裡誤判成「沒切」，見 spec RED 條款。
