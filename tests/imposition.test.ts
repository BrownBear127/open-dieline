...
describe('fitCountStride（profile-spacing spec F2：fitCount 的單一 stride 核心，fitCount 委託此函式）', () => {
  ...
  describe('stride 非正/非 finite 直呼防禦（brief 明文要求——這是 fitCountStride 相對 fitCount 新增的攻擊面：fitCount 委託時 stride=piece+gap 恆為正，但 fitCountStride 本身是公開函式，呼叫端可能直接餵 domain 驗證前的 shrunk.strideY）', () => {
...
// Z_NOTCH_SEGMENTS／Z_NOTCH_GAP／POSITIVE_FILL_INPUT／權威錨值（Z_NOTCH_ANCHOR_DEG0／
// DEG90）定義在 tests/fixtures/z-notch.ts（T3 review Low：與 tests/imposition-preview.
// test.ts 共用同一份，防兩檔各自漂移；幾何設計理由的完整推導見該檔 docblock）。
...
describe('computeImposition — shrunk domain 驗證（profile-spacing spec F2b）', () => {
  ...
  it('shrunk 的 gap 為 NaN（computeProfileStrides 本身不驗證 gap——T1 report 記錄的既知殘留邊界，職責歸本 task；onlyAxis="y" 讓 strideX 明確 undefined，隔離只留一條 stride 錯誤）→ gap 不一致＋shrunkStrideY:not-finite 兩條並存（逐欄收集，非找到第一個就停）', () => {
...
describe('computeImposition — RTE 錨（27"×39" 直放·咬口 20·gap 3·profile-aware 收縮，profile-spacing spec 驗收 1/2/6）', () => {
  ...
  it('0°：行縮擇優，主格點 2×4=8（strideY 收縮把 rows 從矩形排列的 3 撐到 4；strideX 恆等於矩形——RTE 左右緣平直豎線，無收縮空間）', () => {
    ...
    expect(result.deg0.strideY).toBeCloseTo(194.825, 2); // T1 report 獨立重導值（見 tests/profile.test.ts 同一錨）
  ...
  it('兩方向補排皆為 0（spec 驗收 6：該紙規條帶不足容納旋轉件，程式化驗證條帶尺寸本身而不只驗 count）', () => {
    ...
    // 親算條帶尺寸（見 task-2-report.md 完整推導）：spec 源起表的「173.6/116.3」「4.9/6.2」
...
describe('computeImposition — 正數補排案例（人造 Z-notch 幾何，profile-spacing spec 驗收 6：收縮主格點＋條帶補排並存）', () => {
  ...
  it('0°：行縮擇優（strideY=143），gridCount 與「無 shrunk」基線打平（8×2=16）但底條帶多補 2 件（count=18>16）——直接證明 spec F3「比最終 count 不只比 gridCount」的必要性：只看 gridCount 這裡會判兩案平手、漏掉收縮案真正勝出的理由', () => {
    ...
    // toBeCloseTo 斷言（見下一行）——Z_NOTCH_ANCHOR_DEG0 列出 DirectionResult 除
    // utilization 外的全部欄位，鑑別力等同 toEqual 排除 utilization 這一欄。權威數字定義
    // 在 tests/fixtures/z-notch.ts（與 imposition-preview.test.ts 共用，T3 review Low）。
    expect(withShrunk.deg0).toMatchObject(Z_NOTCH_ANCHOR_DEG0);
  ...
  it('90°：列縮擇優（cols 軸用原 strideY=143），gridCount 與基線打平（2×8=16）但右條帶多補 4 件（count=20>16）——同一組輸入的另一方向也成立，證明擇優邏輯不是只在某個特定軸向湊巧成立', () => {
    ...
    // 權威數字定義在 tests/fixtures/z-notch.ts（與 imposition-preview.test.ts 共用，T3 review Low）。
    expect(withShrunk.deg90).toMatchObject(Z_NOTCH_ANCHOR_DEG90);
  ...
  it('間距不變式的最小驗證：收縮後主格點兩軸 stride 皆 ≥ gap（幾何級的完整不變式證明是 Task 5 職責，這裡只驗本 task 輸出的欄位本身沒有低於物理下限——真正撞刀的數字不可能通過這條檢查）', () => {
