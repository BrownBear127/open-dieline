...
// 數值錨的基準輸入（spec 驗收條件 1）：RTE 預設參數的製造 bounds 233.2×251，
...
describe('computeImposition — 數值錨（expected 硬編碼，spec 驗收條件 1）', () => {
...
describe('computeImposition — 對開等式（halfV/halfH＝以「方向處理後」尺寸為整紙的計算）', () => {
  ...
  // resolveWorkingSheet 比較，避免二次 orientation 交換的歧義，見 開發紀錄 決策 6）。
