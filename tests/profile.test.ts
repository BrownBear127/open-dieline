...
describe('computeProfileEnvelope', () => {
  ...
  it('槽邊界雙邊歸屬（人造 V 尖恰在邊界）：單一線段終點恰落在槽 9/10 邊界（x=5.0=10×0.5mm），且該線段定義域完全不進入槽 10——槽 10 若有值，必然是邊界雙歸屬機制本身的效果', () => {
    // 設計理由（mutation testing 找出的坑，見 開發紀錄）：用「V 形兩段線都跨過邊界」
...
describe('computeProfileStrides — RTE 預設件（27"×39"直放·咬口20·gap3 錨）', () => {
  ...
  it('strideY 獨立重導：194.825mm（不設上界——見下方推導與交叉驗證記錄）', () => {
    // 探針 `開發紀錄` 的參考值 193.57 僅為機制參考，不是實作值
    ...
    // arc + 凸包裁切 bezier）算出，經以下獨立重導交叉驗證（過程見 開發紀錄）：
...
describe('ProfileStrides — 封裝（spec 驗收 8）', () => {
  it('型別層：手工物件字面量與 spread 覆寫皆需 @ts-expect-error（由 `npm run typecheck` 的 tsc 驗證；若這裡的 @ts-expect-error 變成「多餘」，tsc 會自己報錯，等於型別 gate 本身會抓到封裝失效）', () => {
    const valid = ProfileStrides.create(3, 236.2, 194.825);

    // @ts-expect-error 手工字面量缺少私有 #brand，不能賦值給 ProfileStrides（型別驗收①）
    const manual: ProfileStrides = { gap: 3, strideX: 236.2, strideY: 194.825 };
  ...
  it('ProfileStrides.create：strideX／strideY 可各自獨立缺省（undefined），getter 忠實回傳（spec F2b 缺省語意——由呼叫端／未來 F3 單向擇優決定要不要帶入某一軸）', () => {
    const onlyY = ProfileStrides.create(3, undefined, 194.825);
    expect(onlyY.gap).toBe(3);
    expect(onlyY.strideX).toBeUndefined();
    expect(onlyY.strideY).toBe(194.825);

    const onlyX = ProfileStrides.create(3, 236.2, undefined);
    expect(onlyX.strideX).toBe(236.2);
    expect(onlyX.strideY).toBeUndefined();

    const neither = ProfileStrides.create(3, undefined, undefined);
    expect(neither.strideX).toBeUndefined();
    expect(neither.strideY).toBeUndefined();
    expect(Object.isFrozen(neither)).toBe(true); // 缺省組合一樣要 freeze，不因欄位缺省而跳過
  });

  it('computeProfileStrides 產出的 instance 兩軸恆為 finite 數字（不缺省——本模組的計算入口一律算滿兩軸，缺省語意留給 Task 2/4 的呼叫端決定）', () => {
    const rect: Segment[] = [
      { kind: 'line', x1: 0, y1: 0, x2: 100, y2: 0 },
      { kind: 'line', x1: 100, y1: 0, x2: 100, y2: 50 },
      { kind: 'line', x1: 100, y1: 50, x2: 0, y2: 50 },
    ...
    const strides = computeProfileStrides(rect, 3);
