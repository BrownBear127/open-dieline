 ...
 * 全管線層，跟 review 實際發現 bug 的量測方式一致：production-P 參數＋telescope.generate()）
...
describe('telescope: F5／F6-B production-P 錨（Slice 5 Task 4 驗收）', () => {
  ...
  it('V relief×4：nominal 5×2.5、每個 2 條 cut 直線、無圓角（feature-normalized 全域計數）', () => {
    ...
    // Slice 5 Task 7 review fix：review 對 F8 block 的等價類 mutation（height 5→4、
    // inset 2.5→2.915475947，保斜邊長不變）驗紅時發現本 block（同一漏洞的另一個落點，
    // 同用 production-P 全參數＋telescope.generate()）同樣只驗斜邊長、同樣會被矇混過關，
    // 補上同一修法（同縫慣例）：逐 V 分組，確認兩線共用頂點，再用 bbox 兩軸獨立鎖
    // height（兩臂端點間距）與 inset（頂點距壁緣）。
    const vGroups: LineSeg[][] = [];
    for (const p of vReliefPaths) {
      const segs = p.segments as LineSeg[];
      for (let i = 0; i < segs.length; i += 2) vGroups.push(segs.slice(i, i + 2));
    }
    expect(vGroups, '4 個 V 分組（每組固定 2 段）').toHaveLength(4);

    for (const [idx, g] of vGroups.entries()) {
      expect(g, `V#${idx} 應恰 2 段`).toHaveLength(2);
      const l1 = g[0]!;
      const l2 = g[1]!;
      const pts = [
        { x: l1.x1, y: l1.y1 },
      ...
      const distinctPts = pts.filter((p, i) => pts.findIndex((q) => Math.abs(q.x - p.x) <= 1e-6 && Math.abs(q.y - p.y) <= 1e-6) === i);
      expect(distinctPts, `V#${idx} 兩線應共用 1 個頂點（apex）——4 端點只有 3 個相異點`).toHaveLength(3);

      const b = segmentsBounds(g);
      const dims = [b.maxX - b.minX, b.maxY - b.minY].sort((a, c) => a - c);
      expect(dims[0], `V#${idx} 頂點距壁緣＝內縮量（bbox 較短軸，獨立於 height）`).toBeCloseTo(2.5, 1);
      expect(dims[1], `V#${idx} 兩臂端點間距＝V 開口高（bbox 較長軸，獨立於 inset）`).toBeCloseTo(5, 1);
    }
  });
...
// re-review 用合法參數（baseLength=34、basePlatformWidth=0，innerWallReduction 沿用
...
describe('telescope: tongue-flap-fits B 款 longWall 門檻同步（re-review concern 1 fix）', () => {
  ...
  it('re-review 實測案例：baseLength=34, basePlatformWidth=0——B-longWall 插底舌已自撞，invariant 現在正確警告（修前用舊 16.5mm 門檻漏報）', () => {
  ...
  it('獨立二分搜尋鎖定真實幾何邊界（不假設任何公式），與 invariant 新門檻互證', () => {
    ...
    // 與本次修正引用的 review 數字（35.396mm）做事後交叉驗證——非本測試的判定依據。
    expect(hi, '獨立二分搜尋收斂值應與 review 給出的門檻數量級一致（事後交叉驗證）').toBeCloseTo(35.396, 2);
...
describe('telescope: F8 production-P 整合錨（Slice 5 Task 7，原檔驅動拓撲測試錨定收攏）', () => {
  ...
  describe('細節三組斷言（spec M6，±0.05）', () => {
    ...
    it('V relief×4：nominal 5×2.5、每個 2 條 cut 直線、無圓角（同 F5/F6-B 錨——這裡在整合層覆核並引用同一 fixture 常數，非複製字面值）', () => {
      ...
      // review fix（Slice 5 Task 7·Medium）：上面的斜邊長檢查對 (height,inset) 是等價類
      // 不敏感——height=4/inset=2.915475947 仍給出同一斜邊長 √(2.915475947²+2²)=3.5355，會
      // 矇混過關（review 等價類 mutation 實證）。逐 V 分組（每 path 內連續 2 段＝1 個 V，見
      // tray.ts buildVReliefPair 逐 endSign 產出 moveTo(arm1).lineTo(apex).lineTo(arm2) 的
      // 建構順序）後：①確認兩線共用 1 個頂點（4 端點只剩 3 個相異點——也順帶覆核分組本身
      // 沒配錯，兩線是不是真的來自同一個 V）②用 bbox 兩軸取值分別鎖 height（兩臂端點間距）
      // 與 inset（頂點距壁緣）兩個獨立維度——同 U-notch 那組斷言一樣 axis-agnostic，不假設
      // 哪個座標軸是 along／perp。
      const vGroups: LineSeg[][] = [];
      for (const p of vReliefPaths) {
        const segs = p.segments as LineSeg[];
        for (let i = 0; i < segs.length; i += details.vRelief.linesPerRelief) {
          vGroups.push(segs.slice(i, i + details.vRelief.linesPerRelief));
        }
      }
      expect(vGroups, '4 個 V 分組（每組固定 2 段）').toHaveLength(featureCounts.lid.vReliefCount);
      ...
      for (const [idx, g] of vGroups.entries()) {
        ...
        const b = segmentsBounds(g);
        const dims = [b.maxX - b.minX, b.maxY - b.minY].sort((a, c) => a - c);
        expect(Math.abs(dims[0]! - details.vRelief.inset), `V#${idx} 頂點距壁緣＝內縮量（bbox 較短軸，獨立於 height）`).toBeLessThanOrEqual(TOL.detail);
        expect(Math.abs(dims[1]! - details.vRelief.height), `V#${idx} 兩臂端點間距＝V 開口高（bbox 較長軸，獨立於 inset）`).toBeLessThanOrEqual(TOL.detail);
      }
    });
