...
// useThickStyle 判準一致。
//
// Slice 5 Fix2（SOL review Finding 1）：本區塊原本誤判 B 款「nominal→offset→offset→
...
describe('gusset 幾何（座標級迴歸）', () => {
  it('style A 校準點（t=0.4/H=60/platform=5）：web 摺線＋對角 cut/crease＋外緣斜切座標', () => {
    ...
    // SOL review M4：先前殘留 height−thickness 舊公式＝59.6，T1 參數遷移漏網已修正，
    // P 實測長壁 outerWall=59.4995 對 59.5 差 0.0005mm ✓，對舊公式 59.6 差 0.1005mm）。
    // y 軸摺線起點在 offset（cornerY+rootJog=89.5+0.5=90.0，Slice 5 Fix2·SOL review
...
describe('generateTray: F6-A 平台端內縮＋角撐周邊複合 relief 鏈（A 款專屬，T0 座標表對算）', () => {
  ...
  it('180° 旋轉實例化：兩組對角配對（left-front↔right-back、right-front↔left-back）座標精確互為負值（角落為旋轉中心，見 buildAGussetChain；Fix 1·2026-07-11 擴充覆蓋第二組對角——先前只驗第一組，相鄰角落誤鏡射的 bug 不會被這條測試單獨抓到）', () => {
    ...
    for (const [cornerA, cornerB] of diagonalPairs) {
      ...
      // task-3-report.md 對算過程——本測試釘住這個關係不被意外破壞）。
  ...
  it('相鄰角落非鏡射（Fix 1·2026-07-11 SOL review H1 迴歸釘樁）：right-front 的平台端 zigzag 深度與 left-front 不同——若誤用 topLeft 模板鏡射四角，這裡會抓到約 20mm 偏差', () => {
  ...
  it('T0 座標表逐項對（left-front／right-front 兩角，±0.05mm）：10° cut、角撐周邊鏈終點（halfcut 邊界）——Fix 1·2026-07-11 擴充覆蓋 right-front（先前只驗 left-front）', () => {
    ...
    // 的 topLeftCorner_*／topRightCorner_* 系列欄位，task-3-report.md／Fix 1 補量記錄）。
    const result = generateTray(baseOpts);
    const T = 0.05;
...
describe('generateTray: F5 B 款舌根拓撲＋V relief（T0 座標表對算）', () => {
  ...
  it('前後壁（shortWall）分支 1：端段 crease 2 段各 35mm、halfcut 76mm（146−2×35）、V relief 4 段（2 個×2 線）', () => {
    ...
    // Slice 5 Task 7 review fix（SOL Medium）：上面的斜邊長檢查對 (height,inset) 是等價類
    // 不敏感——height=4/inset=2.915475947 仍給出同一斜邊長 √(2.915475947²+2²)≈3.5355，會
    // 矇混過關（SOL mutation 實證，同縫修法見 telescope-fixture.test.ts 兩個 V relief
...
describe('telescope: liner 導出鏈（deriveLinerFrame／generateLiner，2026-07-09 T7 gate 反饋重定義：平台式）', () => {
  ...
  // 驗算錨（brief 自檢，controller 手算）：t=0.4/fitGap=0.5/base 179×124/flapDepth=15
  // → 底面 176.4×121.4、攤平 206.4×151.4；t=0.3 預設 → 底面 176.8×121.8。

  it('t=0.4/fitGap=0.5/base 179×124 → 底面 176.4×121.4（brief 驗算錨）', () => {
  ...
  it('thickness=0.3（宣告預設）/fitGap=0.5/base 179×124 → 底面 176.8×121.8（brief 第二驗算錨）', () => {
  ...
  it('同組參數＋flapDepth=15 → 攤平外圍 206.4×151.4（bounds 驗；brief 驗算錨）', () => {
...
describe('telescope: notch-reduced／notch-omitted／platform-corner-omitted（U-notch／平台角可容納降級）', () => {
  ...
  it('S1-S4：未過濾 uNotch／aGussetPeriphery／platformCorner 的 cut 自交零斷言（Fix 2·2026-07-11 SOL review H2——warning 是合法降級聲明，不是自交豁免；S2/S3 降級後 uNotch×aGussetPeriphery 曾在此真自交，被舊版「降級時排除 uNotch」豁免隱藏，這裡刻意不對這三個 tag 套用任何豁免，直接驗證）', () => {
...
// （check().ok===false）的完整集合，不篩子集（2026-07-11 SOL review T5-M1 同型縫修正—
...
describe('telescope: S1-S7 warning 矩陣（spec §驗收 4，B 款新增三條併入）', () => {
   ...
   * （check().ok===false）的 id 完整集合——不篩子集（2026-07-11 SOL review T5-M1 同型縫
  ...
  it('S7（basePlatformWidth=0, lidPlatformWidth=5，款式互換）：{gusset-relief-omitted}（見下方實測更正說明）＋結構斷言（下盒得 B 款舌根拓撲、上蓋得 A 款）', () => {
    ...
    // 根因非本 task 引入的新 bug——A 鏈模板（A_GUSSET_CHAIN_TL/_TR 的固定 T0 內部頂點）
    ...
    // S7 列的 ∅ 需要更正為 {gusset-relief-omitted}（A 鏈本身不在本 task 修改範圍，凍結區）。
