...
describe('generateTray', () => {
  ...
  it('線型斷言：舌摺線含 halfcut 段（中段）與 cut 段（兩端讓位角撐，法蘭裁決需軋斷·2026-07-09 T7 gate）', () => {
    const result = generateTray(baseOpts);
...
describe('telescope: liner 導出鏈（deriveLinerFrame／generateLiner，2026-07-09 T7 gate 反饋重定義：平台式）', () => {
  // 平台式重定義（法蘭裁決，取代圍框版）：底面錨定＝下盒內淨（不再是上蓋內淨），
...
describe('telescope: 全部不變式在預設參數下通過（linerEnabled 開關兩態都驗）', () => {
  ...
  it('t=0 時全部不變式仍通過（全域「補償 t=0 歸零」規範的整體覆蓋；兩態，review Minor 4）', () => {
    // 原本只有 rim-flush 與 liner 導出鏈各自測過 t=0；補一個對全部不變式的迴圈，
    // 讓「t=0 collapse／歸零後幾何仍自洽」在不變式層有整體覆蓋（含 pieces-valid 的
    // 三向 bounds 等式——collapse 少一條線後 hull 仍須閉合）。
    for (const linerEnabled of [true, false]) {
      const params = resolveParams(telescope, { thickness: 0, linerEnabled });
      const result = telescope.generate(params);
      for (const inv of telescope.invariants) {
        expect(inv.check(params, result), `${inv.id}（t=0, linerEnabled=${linerEnabled}）`).toMatchObject({ ok: true });
      }
    }
  });
});
