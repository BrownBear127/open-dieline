...
describe('LayersPanel＋Canvas：點選校準（Slice 3 Task 5 語意延續，校準對象＝選中層）', () => {
  ...
  it('FF1（final review round 2）：校準中點選中層名取消選中 → 校準模式一併關閉，不留殭屍狀態（提示條/表單隨之消失，非只是選不到層）', async () => {
    ...
    expect(screen.queryByText(/點選 overlay 上一段已知長度的線/)).not.toBeInTheDocument();
    expect(screen.queryByLabelText(/該線段實際長度/)).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: '校準' })).toBeInTheDocument(); // 鈕字回「校準」（非「取消校準」）
  });
  ...
  it('FF1 回歸保護：校準中點「另一層」的列名切換選中 → 校準模式維持開啟（既有 T2 F1 語意：切層＝換對象，不是退出）', async () => {
    ...
    expect(within(rowA).getByRole('button', { name: '取消校準' })).toBeInTheDocument();
  });
  ...
  it('FF2（final review round 2）：校準中已選段開表單後隱藏選中層 → 表單消失、確認不可達、scale 不變；重新顯示後可重新點選並正常完成校準', async () => {
    ...
    fireEvent.click(screen.getByLabelText(/顯示疊圖/)); // 表單開著時隱藏選中層
    ...
    fireEvent.click(screen.getByLabelText(/顯示疊圖/)); // 重新顯示
...
describe('Canvas：選中 overlay 層的畫布拖曳（Slice 3 gate round 1 T3，校準>選中拖曳>pan 分流）', () => {
  ...
  it('選中層（visible）＋mousedown(100,100)→mousemove(150,130)：offset 增量=(50/zoom, 30/zoom)，非選中層 offset 不變', async () => {
    ...
    expect(afterB.offsetX - beforeB.offsetX, 'mm delta＝螢幕 delta÷zoom，spec 明文不除 overlay.scale').toBeCloseTo(50 / zoom, 6);
  ...
  it('選中層但已隱藏（!visible）：同序列走 pan 路徑，該層 offset 不變（spec「selectedOverlayId 非 null 且該層 visible」的 visible gate）', async () => {
    render(<App />);
    ...
    const visibleCheckbox = within(rowB).getByLabelText(/顯示疊圖/);
